...
def refresh_entry_and_label(entry_value: int, entry_name: str):
    ...
    print("进入 refresh_entry_and_label() 函数，省份为：", province)
    if province == "外省":
        if category == "文" or category == "理":
            ...
            for c_n in column_names_waisheng:
                column_sum += data_all["服装与服饰设计"][c_n]
                column_sum += data_all["服装与服饰设计"][c_n]
        elif category == "书":
            for c_n in column_names_waisheng:
                column_sum += data_all["书法学"][c_n]
                column_sum += data_all["书法学"][c_n]
        elif category == "音":
            for c_n in column_names_waisheng:
                column_sum += data_all["音乐学"][c_n]
                column_sum += data_all["音乐学"][c_n]
        else:  # "体"
            for c_n in column_names_waisheng:
                column_sum += data_all["社会体育指导与管理"][c_n]
                column_sum += data_all["社会体育指导与管理"][c_n]
        print("外省的 column_sum", column_sum)
        label_object.configure(textvariable=tk.StringVar(value=entry_value - column_sum))

    elif province in ["海南", "山东"]:  # 因为这两个省在 table 里只有一列。
        if category == "综":
...
def process_table_data(event):
    ...
    try:
        ...
        refresh_entry_and_label(entry_value=entry_value, entry_name=str(entry))
    except:
...
def events_handler(event):
    ...
    if "labelframe" in widget_name:       # LabelFrame 里的 Entry
        ...
        try:
            ...
            refresh_entry_and_label(entry_value, entry_name=widget_name)
            """
